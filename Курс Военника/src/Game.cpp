...
void Game::MainLoop() {
    ...
    for (int levelNum = 1; levelNum <= 255; ++levelNum) {
        LoadLevel();
        // в то время как на экране все еще есть точки
...
void Game::MoveGhosts() {
    // проверить изменения режима призрака
...
void Game::UpdateTimers() {
    ...
    // обрабатывать призрака/режим рассеяния
    if (ghostModeTimer) {
        --ghostModeTimer;
